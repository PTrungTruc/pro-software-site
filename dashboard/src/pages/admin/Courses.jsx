...
import { getAdminCourses, updateCourse, deleteCourse, getCourseCategories, getDownloadUrl, uploadThumbnail } from '@/api';
...
import { PlusCircle, Trash2, Loader2, Inbox, RefreshCw, Pencil, Search, Download, ImagePlus, BookOpen } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Progress } from '@/components/ui/progress';

const CourseForm = ({ initialData, onFinish, onCancel }) => {
    ...
    useEffect(() => {
        getCourseCategories().then(res => setCategories(res.data));
        reset(initialData || {});
    }, [initialData, reset]);
    ...
    const onFormSubmit = async (data) => {
        ...
        if (isEditMode) {
            // Logic cập nhật
            const promise = updateCourse(initialData.id, data).finally(() => setIsSubmitting(false));
            ...
            // Logic thêm mới với Upload Chunk (File lớn)
            const file = data.courseFile[0];
            if (!file) {
                toast.error("Vui lòng chọn file giáo trình.");
            ...
            for (let i = 0; i < totalChunks; i++) {
                ...
                try {
                    // Lưu ý: Đảm bảo URL API trỏ đúng về server của bạn
            ...
            if (uploadSuccess) {
                try {
                    await axios.post('http://14.241.225.202:3001/api/assemble-chunks', {
                        ...
                        entity: 'courses', // Quan trọng: Đổi entity thành 'courses'
                        name: data.name,
    ...
    return (
         <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 pt-4 max-h-[80vh] overflow-y-auto px-2">
            ...
            <div className="space-y-2"><Label htmlFor="name">Tên giáo trình</Label><Input id="name" {...register("name", { required: true })} /></div>
            ...
            {!isEditMode && <div className="space-y-2"><Label htmlFor="courseFile">File giáo trình (dung lượng lớn)</Label><Input id="courseFile" type="file" {...register("courseFile")} /></div>}
...
const CoursesAdminPage = () => {
    ...
    const fetchData = async () => { setIsLoading(true); try { const [coursesRes, categoriesRes] = await Promise.all([getAdminCourses(), getCourseCategories()]); setCourses(coursesRes.data); setCategories(categoriesRes.data); } catch (err) { toast.error('Lỗi tải dữ liệu'); } finally { setIsLoading(false); } };
    useEffect(() => { fetchData(); }, []);
    
    const getCategoryName = (categoryId) => categories.find(c => c.id == categoryId)?.name || 'N/A';
    const filteredCourses = useMemo(() => courses.filter(item => (filterCategory === 'all' || item.categoryId == filterCategory)).filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase())), [courses, filterCategory, searchTerm]);
    ...
    const handleDelete = (id) => { if (confirm('Bạn có chắc muốn xóa giáo trình này?')) { const promise = deleteCourse(id).then(() => fetchData()); toast.promise(promise, { loading: 'Đang xóa...', success: 'Xóa thành công!', error: 'Xóa thất bại!' }); } };

    return (
        <div className='pb-[2vh]'>
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6"><h1 className="text-3xl font-bold">Quản lý Giáo trình</h1><div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={fetchData} disabled={isLoading}><RefreshCw className={`mr-2 h-4 w-4 ${isLoading && "animate-spin"}`} /> Làm mới</Button><Button onClick={() => handleOpenDialog(null)}><PlusCircle className="mr-2 h-4 w-4" /> Thêm Mới</Button></div></div>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}><DialogContent className="sm:max-w-2xl"><DialogHeader><DialogTitle>{editingItem ? 'Sửa Thông tin Giáo trình' : 'Thêm Giáo trình Mới'}</DialogTitle></DialogHeader><CourseForm initialData={editingItem} onFinish={handleFormSuccess} onCancel={handleCloseDialog} /></DialogContent></Dialog>
                ...
                <TableBody><AnimatePresence>
                    ...
                    : filteredCourses.map(item => (
                        <motion.tr key={item.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -50 }}>
                            <TableCell>{item.thumbnail ? <img src={getDownloadUrl(item.thumbnail)} alt={item.name} className="w-12 h-12 object-cover rounded-md"/> : <div className="w-12 h-12 bg-muted rounded-md flex items-center justify-center"><BookOpen className="h-6 w-6 text-muted-foreground"/></div>}</TableCell>
                    ...
                    {!isLoading && filteredCourses.length === 0 && <TableRow><TableCell colSpan={5} className="text-center h-24"><Inbox className="mx-auto h-8 w-8 text-muted-foreground" /><p className="text-muted-foreground mt-2">Không tìm thấy giáo trình nào.</p></TableCell></TableRow>}
